...
3. 当前 [fifo/tests/test_reset.py](fifo/tests/test_reset.py) 已经有功能覆盖率定义，本次任务不修改这部分逻辑。
4. 最终报告页面中会展示 Line / Functions / Branches 三类代码覆盖率指标，但它们的前提是当前 `DUTSyncFIFO` 构建产物本身已经包含相应 coverage 数据。

因此，本次工作不重写 reset 测试逻辑，不调整功能覆盖率逻辑，只在现有结构上补齐 code coverage 接线。

## 目标

在不改变现有 reset 用例语义与功能覆盖率实现的前提下，实现以下结果：

1. 运行 `test_reset` 时，可由 `toffee_test --toffee-report` 汇总生成 line / function / branch 覆盖率。
2. 保留现有 reset 功能覆盖点 `RST-01` / `RST-02` / `RST-03`，且不修改其定义、注册和采样逻辑。
3. 改动尽量局限在 [fifo/tests/test_reset.py](fifo/tests/test_reset.py)；仅在必要时补充最小运行配置。

## 方案选择

### 方案 A（采用）
在 [fifo/tests/test_reset.py](fifo/tests/test_reset.py) 的 fixture 中，只补 `toffee_request.create_dut(...)` 这条路径对应的 code coverage 导出与报告生成能力，保持现有功能覆盖率代码原样不动。

这样 `toffee_test` 结束测试时会自动：

- 收集 DUT 生成的 `.dat`
- 合并代码覆盖率报告
- 保留现有 `CovGroup` 功能覆盖结果

### 未采用方案

- **调整功能覆盖率注册方式**：虽然可以顺手整理，但超出本次任务范围。
...
当前文件中已经存在 `get_cover_group_fifo_state(...)`、功能覆盖点 `RST-01` / `RST-02` / `RST-03`，以及对应的异步采样任务。
...
这样可以把改动范围严格限制在代码覆盖率接线本身，避免把“补代码覆盖率”扩展成“重构功能覆盖率实现”。

### 3. 对当前代码结构的最小调整

预计只做以下结构性调整：

1. 为 `test_reset` 这条测试路径确认并接通 code coverage 输出。
2. 仅在需要时补充最小的运行参数或 DUT 覆盖率输出配置。
3. 不改动现有功能覆盖率相关代码。
...
6. fixture teardown 时 `toffee_test` 同时回收：
   - 现有功能覆盖结果
   - 代码覆盖数据
7. `toffee_test` 统一生成报告
...
2. **功能覆盖未命中**：不影响测试是否通过，但会在报告里体现为功能覆盖缺口；这属于现有测试质量信号，不是本次实现要修改的范围。
...
3. 确认报告目录中存在 line coverage 数据页面，且页面包含：
   ...
   - Functions
   - Branches
4. 确认现有功能覆盖组仍显示 `RST-01` / `RST-02` / `RST-03`，且无需代码改动。
