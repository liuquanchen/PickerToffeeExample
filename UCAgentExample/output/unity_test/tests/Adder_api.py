...
from Adder import DUTAdder  # Replace with the actual DUT class import
...
def get_coverage_data_path(request, new_path:bool):
    ...
    tc_name = request.node.name if request is not None else "Adder"
...
def create_dut(request):
    """
    Create a new instance of the Adder for testing.
    
    Returns:
        dut_instance: An instance of the Adder class.
    """
    # 如果是正在生成测试模板，返回fake DUT用于提速（模板中不会真运行DUT）
    if ucagent.is_imp_test_template():
        return ucagent.get_fake_dut(DUTAdder)
...
def dut(request):
    ...
    # 设置用户信息到报告
    set_user_info("UCAgent-26.4.18.dev28+ge522f2188", "unitychip@bosc.ac.cn")
    set_title_info("Adder Test Report")
...
def mock_dut():
    return ucagent.get_mock_dut_from(DUTAdder)
...
# def api_Adder_{operation_name}(env, ...):
#    """
